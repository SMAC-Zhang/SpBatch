...
class BasicDataset(Dataset):
    def __init__(self, X, Y, n, train, device):
...
def get_data(args, l):
    if CONFIG[args.model]['ckt_storage'] == "bylayer":
        ...
        query = np.array(np.memmap(path, dtype='float16', mode='r', shape=(SAMPLE_NUM, CONFIG[args.model]['d'])))[:CONFIG[args.model]['N']]
    
        path = f"{args.dataset}/mlp_label_{l}.mmap"
        print(f"Reading MLP label from {path}")
        label = np.array(np.memmap(path, dtype='float16', mode='r', shape=(SAMPLE_NUM, CONFIG[args.model]['f'])))[:CONFIG[args.model]['N']]
    
        return  query, label


def create_dataset(query, labels, args, device):
    ...
    train_ds = BasicDataset(query, labels, num_train, True, device)
    test_ds = BasicDataset(query, labels, num_test, False, device)
...
def main():
    ...
    train_loader, test_loader = create_dataset(query, labels, args, device)
